...
class CuckooFilter {
  // Storage of items
public:
  std::shared_ptr<TableType<bits_per_item>> table_;
  ...
  inline void GenerateIndexTagHash(const ItemType& item, size_t* index,
                                   uint32_t* tag) const {
    uint64_t hv = std::hash<std::string>{}(item);
    const uint64_t hash = hasher_(hv);
    // const uint64_t hash = hasher_(item);

    // const uint32_t hash = hasher_.MurmurHash(item);
 ...
 public:
  explicit CuckooFilter(const size_t max_num_keys,vector<unsigned __int128> random_vector) : num_items_(0), victim_(), hasher_(random_vector) {
    size_t assoc = Constants::K_TAGS_PER_BUCKET;
    ...
    victim_.used = false;
    table_ = std::make_shared<TableType<bits_per_item>>(num_buckets);
  }

  CuckooFilter() : num_items_(0), victim_(), hasher_() {
...
template <typename ItemType, size_t bits_per_item,
          template <size_t> class TableType, typename HashFamily>
Status CuckooFilter<ItemType, bits_per_item, TableType, HashFamily>::Rebalance(){
  // 最大kick次数，什么时候触发rebalance
  size_t rebalance_tag = std::floor(Constants::REBALANCE_THRESHOLD * table_->kTagsPerBucket);
  size_t old_tagPerBucket = table_->kTagsPerBucket;
  // cout << " old tag per bucket " << old_tagPerBucket << endl;
  double old_loadfactor = LoadFactor();
  // cout << " rebalance_tag " << rebalance_tag << endl;
  // cout << " load factor " << LoadFactor() << endl;
  // cout << " number item " << Size() << endl;

  
  if ( old_loadfactor <= Constants::REBALANCE_THRESHOLD * 0.95 && old_tagPerBucket > 1){
    table_->kTagsPerBucket = rebalance_tag;
    for(size_t i = 0; i < table_->num_buckets_; ++i){
      for(size_t j = rebalance_tag; j < old_tagPerBucket; ++j){
        uint32_t tag = table_->ReadTag(i+j*table_->num_buckets_, 0);
        if(tag != 0){
          if (victim_.used) {
            table_->kTagsPerBucket = old_tagPerBucket;
            return NotEnoughSpace;
          }
          Status res = AddImpl(i, tag);
          table_->WriteTag(i+j*table_->num_buckets_, 0, 0);
          num_items_--;
          if(res != Ok){
            table_->kTagsPerBucket = old_tagPerBucket;
            return res;
    ...
    // delete tag
    table_->ResizeTable(rebalance_tag);
  }
  return Ok;
}
}  // namespace cuckoofilter
